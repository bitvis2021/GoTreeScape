...
def compute_vector_dist(subtree_avg_latent_vector, tree_vis_latent_vector):
    '''
        compute the distances between different vectors
    '''
...
def compute_deep_cluster_nearest_neighbor():
    '''
        the argument of this function is the dsl object and the model compute the latent vector according to the declarative grammar,
        then locate the position according to the latent vector
    '''
    pass

def compute_nearest_neighbor_from_interior_cluster(target_tree_vis_latent_vector, cluster_content, tree_vis_index, displayed_level):
    '''
    ...
    # the amount of selected dsl object is relative to the displayed level
    neighbor_amount = neighbor_amount // displayed_level
    # cluster_content = []
    # interior_cluster_index = str(interior_cluster_index)
...
# def  (max_displayed_level, displayed_level, target_treevis_index):
#     '''
...
def find_neighbor_representative(target_treevis_index, treevis_index_list, displayed_level):
    '''
    '''
    target_tree_vis_latent_vector = latent_vector_list[target_treevis_index]
    target_tree_vis_latent_vector = transform_list_str_to_float(target_tree_vis_latent_vector)
    nearest_neighbor_index_list = compute_nearest_neighbor_from_interior_cluster(target_tree_vis_latent_vector, treevis_index_list, target_treevis_index, displayed_level)
    return nearest_neighbor_index_list

def traverse_find_neighbor_representative(displayed_level, target_treevis_index, cluster_tree, displayed_data_range):
    '''
        traverse the cluster tree and find neighbor representatives
    '''
    node_depth = cluster_tree["depth"]
    if displayed_level <= (node_depth - 1):
        representative_list = cluster_tree["representative"]
        inner_leaf_list = cluster_tree["inner_leaf"]
        return find_neighbor_representative(target_treevis_index, inner_leaf_list, displayed_level)
    else:
        left_neighbor_representative = []
        right_neighbor_representative = []
        if "left" in cluster_tree:
            left_tree_pos_range = cluster_tree['left']['pos_range']
            left_tree_pos_state = compute_relative_pos_state(displayed_data_range, left_tree_pos_range)
            if left_tree_pos_state != 'displayed_range_apart_cluster_range':
                left_neighbor_representative = traverse_find_neighbor_representative(displayed_level, target_treevis_index, cluster_tree['left'], displayed_data_range)
        if 'right' in cluster_tree:
            right_tree_pos_range = cluster_tree['right']['pos_range']
            right_tree_pos_state = compute_relative_pos_state(displayed_data_range, right_tree_pos_range)
            if right_tree_pos_state != 'displayed_range_apart_cluster_range':
                right_neighbor_representative = traverse_find_neighbor_representative(displayed_level, target_treevis_index, cluster_tree['right'], displayed_data_range)
        neighbor_representative = left_neighbor_representative + right_neighbor_representative
        random.shuffle(neighbor_representative)
        return neighbor_representative

def compute_neighbor_representative_new(displayed_level, target_treevis_index, cluster_tree, displayed_data_range):
    '''
        compute the neighbor representative items of the target tree vis index by traversing the hierarchical data
    '''
    displayed_level = 5
    neighbor_representative_list = traverse_find_neighbor_representative(displayed_level, target_treevis_index, cluster_tree, displayed_data_range)
