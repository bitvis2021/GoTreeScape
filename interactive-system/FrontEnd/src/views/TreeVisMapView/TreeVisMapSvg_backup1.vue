    ...
    export default {
            ...
            selectedDatasetFilterList: function() {
                ...
                if (uploadingFileDatasetFilterObjList.length === 0) {
                    ...
                    $.when(locateUploadingFileDefer)
                        .done(function() {
                            // let cluster_index = searching_results['cluster_index']
       //                   let cluster_inner_neighbor = searching_results['cluster_inner_neighbor']
            ...
            showLandmarkPoint: function() {
                console.log('update showLandmarkPoint', this.showLandmarkPoint)
                let zoomEnd = true
                this.updateWholeProjectionView(this.transformEvent, zoomEnd)
            },
            showLandmarkPreview: function() {
                console.log('update showLandmarkPreview', this.showLandmarkPreview)
                this.updatePreviewRepresentativeList()
                this.arrangePreviewRepresentativeObjList()
            },
            showExistTreePoint: function() {
                console.log('update showExistTreePoint', this.showExistTreePoint)
                let zoomEnd = true
                this.updateWholeProjectionView(this.transformEvent, zoomEnd)
            },
            showExistTreePreview: function() {
                console.log('update showExistTreePreview', this.showExistTreePreview)
                this.updatePreviewRepresentativeList()
                this.arrangePreviewRepresentativeObjList()
            },
     ...
     //             self.loadClusterResults(loadClusterDefer)
            ...
            updateZoomingRatio: function(transform_event) {
                ...
                $.when(loadClusterDefer)
                    .done(function() {
                        // console.log("===============load cluster results finished=================", new Date())
                        let zoom_end = true
                        self.updateUnderlyingRepresentativeDataset()
                ...
                // self.updateWholeProjectionView(transform_event, true)
                // console.log("===============load cluster results start=================", new Date())
            ...
            updateProjectionContour: function (transform_event) {
                ...
                let treeVisMapCanvasWholeHeight = self.treeVisMapCanvasWholeHeight
                let treeVisMapCanvasWholeWidth = self.treeVisMapCanvasWholeWidth
                for (let itemIndex = 0; itemIndex < clusterContentItemList.length; itemIndex++) {
                    let projectionPos = clusterContentItemList[itemIndex]
                    let projectionPosUpdate = transform_event.apply(projectionPos)
                    let projectionPosX = projectionPosUpdate[0]
                    let projectionPosY = projectionPosUpdate[1]
                    // if the node positions exceed the screen space, then remove the nodes
                    if ((projectionPosX >= 0) && (projectionPosX <= treeVisMapCanvasWholeWidth) && (projectionPosY >= 0) && (projectionPosY <= treeVisMapCanvasWholeHeight)) {
                        clusterContentItemListRemained.push(projectionPos)
                    }
                }
                // update the contour
                ...
                console.log('end render contour')
            },
            ...
            updateUnderlyingRepresentativeDataset: function() {
            ...
            computeDisplayAndDataRange: function() {
                ...
                if ((self.xScaleRevert != null) && (self.yScaleRevert != null)) {
                    ...
                    // treeVisMapCanvasWidth, treeVisMapCanvasHeight
                    displayRange = {'x': [displayedMinX, displayedMaxX], 'y': [displayedMinY, displayedMaxY]}
                    // compute ratio of the display range
                    displayRangeRatio = {
                        'x': [displayedMinX / xRangeMax, displayedMaxX / xRangeMax], 
                        'y': [displayedMinY / yRangeMax, displayedMaxY / yRangeMax]
                    }
                } 
            ...
            zoomChangeExceedThreshold: function(displayAndDataRangeBeforeUpdate, displayAndDataRange) {
                ...
                // console.log('displayedRangeBeforeUpdate', displayedRangeBeforeUpdate, 'dataRangeBeforeUpdate', dataRangeBeforeUpdate)
            ...
            loadClusterResults: function(deferObj) {
                ...
                let formData = {'level': displayedLevel, 'zoomRatio': zoomingRatio, 'treevisIndex': locateTreeVisIndex, 'displayedDataRange': self.displayedDataRange, 'type': loadRepresentativeItemType}
            ...
            updatePreviewRepresentativeList: function(random_state=false) {
                ...
                if (self.showLandmarkPreview) {
                    for (let i = 0; i < previewClusterRepresentativeIndexList.length; i++) {
                        ...
                        if (existedTreeVisIndexList.indexOf(treevisIndex) === -1) {
            ...
            zoomEndHandler: function () {
                let self = this
                // the handler for zooming function
                var transformEvent = d3.event.transform
                var transformRatio = transformEvent.k
                // update the global transformEvent
                self.transformEvent = transformEvent
                // console.log('=======zoomEndHandler========', transformEvent, new Date())
                let displayAndDataRangeBeforeUpdate = self.currentDisplayAndDataRange
                let displayAndDataRange = self.computeDisplayAndDataRange()
                if ((displayAndDataRangeBeforeUpdate == null) || (self.zoomChangeExceedThreshold(displayAndDataRangeBeforeUpdate, displayAndDataRange))) {
                    self.currentDisplayAndDataRange = displayAndDataRange
                    self.displayRangeRatio = displayAndDataRange['displayRatio']
